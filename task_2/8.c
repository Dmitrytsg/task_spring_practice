/*Функцию сортировки по значению числового 
поля методом прямого выбора по убыванию.
Сортировка по полю года выпуска*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdlib.h>
...
void sorting(music **song_00,int num);
...
void sorting(music **song_00,int num)
{
    ...
    for(int i=0;i<num-1;i++)
    {
        ...
        {
            if(song_00[j]->year_of_rels>song_00[max]->year_of_rels) max = j;
        }
        temp = song_00[i];
        song_00[i] = song_00[max];
        song_00[max] = temp;
    }
}